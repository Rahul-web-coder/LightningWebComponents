...
export default class PdfGenerationDemo extends LightningElement {
    ...
    imageUrl = 'https://cloudkaptan67-dev-ed--c.develop.vf.force.com/resource/1741591268000/logo'
    invoiceData = {
        invoiceNo: '123',
    ...
    pdfHandler(){
        ...
        generatePDF({ recordId: this.recordId , htmlData: content.outerHTML}).then(result=>{
            ...
            window.open(`https://cloudkaptan67-dev-ed.develop.file.force.com/servlet/servlet.FileDownload?file=${result.Id}`);
        }).catch(error=>{
            console.error(error);
        })  
    }
